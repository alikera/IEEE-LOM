...
def process_interactivity_level_tag(lom, slide):
...
def calculate_semantic_density(curr_slide):
    slide_text = ""
    each_slide_text = ""
    densities = []
...
def calculate_semantic_density2(curr_slide):
...
def process_semantic_density_tag(lom, slide):
    semantic_density_score = calculate_semantic_density(slide)
    lom["semanticDensity"] = semantic_density_score


def process_intended_end_user_role_tag(lom, slide):
    lom['intendedEndUserRole'] = 'learner'

def process_context_tag(lom, slide):
...
def calculate_difficulty(curr_slide):
...
def process_difficulty_tag(lom, slide):
    difficulty = calculate_difficulty(slide)
    lom['difficulty'] = difficulty
    

def calculate_age_estimation(curr_slide):
    ppt = pptx.Presentation("assets/"+curr_slide)
    text = extract_text_from_slide(ppt)
...
def calculate_age_range(slide):
    age_estimated = calculate_age_estimation(slide)
    difficulty = calculate_difficulty(slide)
    age_mid = int((age_estimated + difficulty)/2)

    return age_mid + 7
def process_typical_age_range_tag(lom, slide):
    age = calculate_age_range(slide)
...
def calculate_learning_time(lom, curr_slide):
    ppt = pptx.Presentation("assets/"+curr_slide)
    text = extract_text_from_slide(ppt)
...
def process_typical_learning_time(lom, slide):
    reading_time = calculate_learning_time(lom, slide)
    print(reading_time)

    lom['typicalLearningTime'] = reading_time


def process_description_tag(lom, slide):
    lom['description'] = ""
...
def process_language_tag(lom, slide):
...
for slide in my_slides:
    ...
    process_interactivity_level_tag(lom, slide)
    process_semantic_density_tag(lom, slide)
    process_intended_end_user_role_tag(lom, slide)
    process_context_tag(lom, slide)
    process_typical_age_range_tag(lom, slide)
    process_difficulty_tag(lom, slide)
    process_typical_learning_time(lom, slide)
    process_description_tag(lom, slide)
    process_language_tag(lom, slide)
    # calculate_semantic_density(slide)
    LOMs.append(lom)
